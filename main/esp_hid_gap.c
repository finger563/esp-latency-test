...
void print_uuid(const esp_bt_uuid_t *uuid) {
  if (uuid->len == ESP_UUID_LEN_16) {
    ...
    GAP_DBG_PRINTF(
        ...
        uuid->uuid.uuid128[8], uuid->uuid.uuid128[9], uuid->uuid.uuid128[10],
        uuid->uuid.uuid128[11], uuid->uuid.uuid128[12], uuid->uuid.uuid128[13],
        uuid->uuid.uuid128[14], uuid->uuid.uuid128[15]);
  }
}
...
static void handle_bt_device_result(struct disc_res_param *disc_res) {
  ...
  for (int i = 0; i < disc_res->num_prop; i++) {
    ...
    if (prop->type == ESP_BT_GAP_DEV_PROP_BDNAME) {
      ...
      GAP_DBG_PRINTF("major: %s, minor: %d, service: 0x%03x", esp_hid_cod_major_str(cod->major),
                     cod->minor, cod->service);
...
static void handle_ble_device_result(struct ble_scan_result_evt_param *scan_rst) {
  ...
  uint8_t *adv_name =
      esp_ble_resolve_adv_data(scan_rst->ble_adv, ESP_BLE_AD_TYPE_NAME_CMPL, &adv_name_len);
...
static void handle_ble_device_result(const struct ble_gap_disc_desc *disc) {
  ...
  for (int i = 0; i < fields.num_uuids16; i++) {
    if (ble_uuid_u16(&fields.uuids16[i].u) == BLE_HID_SVC_UUID &&
      ...
      add_ble_scan_result(disc->addr.val, disc->addr.type, appearance, adv_name, adv_name_len,
                          disc->rssi);
      break;
    }
  }
...
static esp_err_t start_nimble_scan(uint32_t seconds) {
  ...
  rc = ble_gap_disc(own_addr_type, seconds * 1000, &disc_params, nimble_hid_gap_event, NULL);
  if (rc != 0) {
